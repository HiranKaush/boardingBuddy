    ...
    const pickImage = async () => {
      ...
      const uploadMedia = async () => {
        ...
        try {
          ...
          const ref = firebase.storage().ref().child(`store/${filename}`);
      ...
      const UpdateStoreProfile = async () => {
        try {
            console.log("here")
            console.log(imgURL)
          const response = await axios.post(`http://192.168.1.13:8000/api/store/updateStoreImage/${userId}`, {imgURL: imgURL});
   ...
   >
     <View style={styles.centeredView}>
       <View style={styles.modalView}>
         <Text style={styles.modalText}>Add Store picture</Text>
      ...
      <Button title="Add Item" onPress={UpdateStoreProfile} />
    </View>
    )
};
